/* ═══════════════════════════════════════════════════════════════════
   aggregate.js v3.1 — 1-based номери фото для UI

   Зміни v3 → v3.1:
   - Усі номери фото в тексті note конвертуються з 0-based → 1-based
     перед показом користувачу. "Видно на фото 0" → "Видно на фото 1".
   - Внутрішньо JS і AI продовжують працювати в 0-based.

   Pipeline:
   1. AI пише: "Видно на фото 0: зліва на фото бампер пробитий"
   2. analyzeZoneGroup ремапить batch_idx → original_idx (0-based)
   3. aggregate перекладає сторону фото → сторона авто
   4. aggregate бампає всі номери +1 для UI
   5. Користувач бачить: "Видно на фото 1: правий бік авто (пасажирський) бампер пробитий"
   ═══════════════════════════════════════════════════════════════════ */
...
export function aggregate(opts) {
  ...
  if (classifications) {
    classifications.forEach(function (c) {
      photoRoles[c.photo_index] = c.role
    })
  }
  ...
  relevantZones.forEach(function (zone) {
    ...
    var descriptionItems = buildZoneDescriptionItems(problems, intactCps, unclearCps, photoRoles)
...
/* ═══ Description builder — переклад сторін + 1-based номери ═══ */
function buildZoneDescriptionItems(problems, intactCps, unclearCps, photoRoles) {
  ...
  problems.forEach(function (p) {
    ...
    /* КРОК 1: переклад сторін фото → сторони авто (поки що в 0-based номерах) */
    text = translatePhotoSideToCarSide(text, photoRoles)

    /* КРОК 2: bump номерів +1 для UI (0-based → 1-based)
       Це робимо ОСТАННІМ — після перекладу сторін, бо translate шукає номер фото
       щоб дізнатись роль (а в photoRoles ключі — 0-based) */
    text = bumpPhotoNumbers(text)
...
/* Переклад сторони фото → сторона авто. Працює з 0-based номерами */
function translatePhotoSideToCarSide(noteText, photoRoles) {
  var photoMatch = noteText.match(/Видно на фото\s*([\d,\s]+)/i)
  if (!photoMatch) return noteText

  var photoNums = photoMatch[1].match(/\d+/g)
  if (!photoNums || photoNums.length === 0) return noteText

  var firstPhotoIdx = parseInt(photoNums[0])
  var role = photoRoles[firstPhotoIdx] || null

  if (!role) return noteText

  var translateLeft, translateRight
  if (role === 'front_full' || role === 'front_close') {
    translateLeft = 'правий бік авто (пасажирський)'
    translateRight = 'лівий бік авто (водійський)'
  } else if (role === 'rear_full' || role === 'rear_close') {
    translateLeft = 'лівий бік авто (водійський)'
    translateRight = 'правий бік авто (пасажирський)'
  } else if (role === 'side_left_full' || role === 'side_left_close') {
    return noteText
  } else if (role === 'side_right_full' || role === 'side_right_close') {
    return noteText
  } else {
    return noteText
  }
  ...
  return noteText
    .replace(/зліва на фото|у лівій частині фото|в лівій частині фото|ліворуч на фото/gi, translateLeft)
    .replace(/справа на фото|у правій частині фото|в правій частині фото|праворуч на фото/gi, translateRight)
}

/* Знаходить "фото X" або "фото X, Y, Z" і збільшує кожен номер на 1.
   "Видно на фото 0" → "Видно на фото 1"
   "Видно на фото 0, 4" → "Видно на фото 1, 5" */
function bumpPhotoNumbers(text) {
  return text.replace(/фото\s*([\d,\s]+)(?=:|\b)/gi, function (match, numsStr) {
    var nums = numsStr.match(/\d+/g)
    if (!nums) return match
    var bumped = nums.map(function (n) { return (parseInt(n) + 1) })
    return 'фото ' + bumped.join(', ')
  })
}
...
function extractPhotoNumbers(noteText, fallbackPhoto) {
  ...
  if (typeof fallbackPhoto === 'number') return [fallbackPhoto + 1]  /* +1 теж тут для консистентності UI */
  return []
}
